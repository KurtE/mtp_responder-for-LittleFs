...
//#ifdef USE_SPI
  ...
  void storage_configure_spi(MTPStorage_SPI *storage1, const char **sd_str, const int *cs, LittleFS_SPIFlash *sdx, int num)
  {
...
//#elif defined(USE_RAM)
  ...
  void storage_configure_ram(MTPStorage_RAM *storage2, const char **ram_str, const int *ramIdx, LittleFS_RAM *sdp, int num)
  {
...
//  void storage_configure_qspi(MTPStorage_QSPI *storage3, const char **qspi_str, const int *qspiIdx, LittleFS_QSPIFlash *sdr, int num)
//  {
...
{ 
  ...
  usb_mtp_configure();
  //#ifdef USE_RAM
    storage_configure_ram(&storage2, ram_str,ramIdx, sdp, nsdRam);
  //#elif defined(USE_SPI)
    storage_configure_spi(&storage1, sd_str,cs, sdx, nsd);
  //#elif defined(USE_QSPI)
  //    storage_configure_qspi(&storage3, qspi_str, qspiIdx, sdr, nsdQSPI);
  //#endif
